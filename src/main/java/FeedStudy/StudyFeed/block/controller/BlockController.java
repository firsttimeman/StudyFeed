...
import FeedStudy.StudyFeed.block.dto.BlockListResponse;
...
public class BlockController {
    ...
    public ResponseEntity<String> blockUser(@AuthenticationPrincipal User user,
                                            @Parameter(description = "차단할 유저의 ID") @PathVariable Long otherId) {
    ...
    public ResponseEntity<String> unblockUser(@AuthenticationPrincipal User user,
                                              @Parameter(description = "차단 해제할 유저의 ID") @PathVariable Long otherId) {
    ...
    @GetMapping
    public ResponseEntity<BlockListResponse> getBlocks(@AuthenticationPrincipal User user) {
        List<BlockSimpleDto> blocks = blockService.blockList(user);
        return ResponseEntity.ok(BlockListResponse.from(blocks));
    }
